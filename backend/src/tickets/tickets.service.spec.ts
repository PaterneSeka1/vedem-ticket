...
describe('TicketsService', () => {
  ...
  it('generates one ticket per unit ordered, each with a unique code', async () => {
    const tickets = await service.generateForOrder({
      id: 'order-1',
      items: [{ ticketCategoryId: 'cat-1', quantity: 3 }],
    });
  ...
  it('generates tickets for every item when the order spans several categories', async () => {
    const tickets = await service.generateForOrder({
      id: 'order-1',
      items: [
    ...
    expect(tickets).toHaveLength(3);
    expect(tickets.filter((t: any) => t.ticketCategoryId === 'cat-1')).toHaveLength(2);
    expect(tickets.filter((t: any) => t.ticketCategoryId === 'cat-2')).toHaveLength(1);
  });

  it('is idempotent: calling it twice for the same order does not duplicate tickets', async () => {
    await service.generateForOrder({ id: 'order-1', items: [{ ticketCategoryId: 'cat-1', quantity: 2 }] });
    const second = await service.generateForOrder({
      id: 'order-1',
  ...
  it('findAll() returns every ticket across orders', async () => {
    await service.generateForOrder({ id: 'order-1', items: [{ ticketCategoryId: 'cat-1', quantity: 2 }] });
    await service.generateForOrder({ id: 'order-2', items: [{ ticketCategoryId: 'cat-1', quantity: 1 }] });
